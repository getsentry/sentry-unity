...
public class DebugSymbolUploadTests
{
    ...
    public void AppendUploadToGradleFile_AllRequirementsMet_AppendsUploadTask(bool isExporting, bool addMapping, bool ignoreCliErrors)
    {
        ...
        var keywords = new List<string>
        {
            ...
            "sentry.properties",
            "tasks.register('sentryUploadDebugFiles', Exec)",
            "args = ['debug-files', 'upload', '--il2cpp-mapping', '--include-sources'",
            "tasks.register('sentryUploadSymbols')",
            "dependsOn 'sentryUploadDebugFiles'",
            "tasks.named('assembleDebug').configure { finalizedBy 'sentryUploadSymbols' }",
            "tasks.named('assembleRelease').configure { finalizedBy 'sentryUploadSymbols' }",
            "tasks.named('bundleDebug').configure { finalizedBy 'sentryUploadSymbols' }",
            "tasks.named('bundleRelease').configure { finalizedBy 'sentryUploadSymbols' }"
        };

        if (!isExporting)
        {
            keywords.Add(DebugSymbolUpload.SymbolUploadLogName);
            keywords.Add("standardOutput = new org.apache.tools.ant.util.TeeOutputStream(System.out, sentryLogFile)");
        }
        ...
        if (addMapping)
        {
            keywords.Add("tasks.register('sentryUploadProguardMapping', Exec)");
            keywords.Add("args = ['upload-proguard'");
            keywords.Add("dependsOn 'sentryUploadProguardMapping'");
            if (!isExporting)
            {
                keywords.Add(DebugSymbolUpload.MappingUploadLogName);
                keywords.Add("standardOutput = new org.apache.tools.ant.util.TeeOutputStream(System.out, mappingLogFile)");
            }
        }
    ...
    public void RemoveUploadTaskFromGradleFile_MultipleConsecutiveRemovals_HandlesCorrectly()
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            ...
            StringAssert.DoesNotContain("sentryUploadSymbols", gradleFile);

            sut.AppendUploadToGradleFile(_fixture.SentryCliPath);

            gradleFile = File.ReadAllText(GetGradleFilePath());
            StringAssert.Contains("sentryUploadSymbols", gradleFile);
