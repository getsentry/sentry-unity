...
- Switch and Switch 2 players no longer link Sentry's no-op stubs when sentry-switch is installed. The stubs used to ship as a plugin inside the package, enabled by default, with a build callback turning them off once the real libraries appeared. That write never landed for a package installed from a registry or a Git URL, because Unity treats those as immutable, so the linker kept preferring the stubs and the player shipped without native crash reporting while the build log reported the libraries as found. The stubs are now copied into `Assets/Plugins/Sentry/<target>` only while that target's libraries are missing, and removed again once they are there. Only the active build target gets a copy, so projects that do not build for Switch see no change in their tree. The copy carries a version stamp, so an SDK upgrade that changes the stubs refreshes it. ([#2849](https://github.com/getsentry/sentry-unity/pull/2849))
- Three Switch stubs returned `void` where sentry-native returns `int`: `sentry_close`, `sentry_value_decref` and `sentry_reinstall_backend`. A caller reading the result of any of them got whatever happened to be in the return register. Nothing reads them today, so no behaviour changes, but a stubbed player no longer disagrees with a real one about the calling convention. ([#2849](https://github.com/getsentry/sentry-unity/pull/2849))
