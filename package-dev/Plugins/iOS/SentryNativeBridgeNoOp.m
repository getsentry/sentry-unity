...
#import <Sentry/Sentry.h>  // Import the Sentry framework headers
...
{
    if (loadStatus == -1) {
        loadStatus = 1; // Always succeed as framework is linked
    }
    return loadStatus;
}
...
void SentryNativeBridgeStartWithOptions(const void *options)
{
    ...
    NSError *error = nil;
    id sentryOptions = [[SentryOptions alloc] initWithDict:dictOptions didFailWithError:&error];
    
    if (error) {
        NSLog(@"Sentry (bridge): failed to initialize options: %@", error.localizedDescription);
        return;
    }
